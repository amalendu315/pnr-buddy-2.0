...
const formatExcelDate = (inputDate: any) => {
  ...
  // 3. JS Date Objects (Bug Fixed)
  if (inputDate instanceof Date) {
    // If the date is perfectly aligned to UTC midnight (often caused by string parsing)
    // we should format it using UTC to avoid negative timezone shifts.
    if (inputDate.getUTCHours() === 0 && inputDate.getUTCMinutes() === 0) {
      return moment.utc(inputDate).format("YYYY-MM-DD");
    }

    // For everything else, including dates/times generated by XLSX, 
    // trust the local moment parsing to extract the correct YYYY-MM-DD natively 
    // without arbitrarily adding 12 hours.
