...
type Airline = "spicejet" | "indigo";
type FormatType = "goodBad" | "purchaseData";

export default function AirlineFormatPage() {
  const [formatType, setFormatType] = useState<FormatType>("goodBad");
  const [airline, setAirline] = useState<Airline>("spicejet");
  const [email, setEmail] = useState("");
  const [file, setFile] = useState<File | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    formData.append("airline", airline);
    formData.append("email", email);
    formData.append("formatType", formatType); // Send the format type to the backend
        ...
        <div className="md:w-3/5 p-8 lg:p-12">
          ...
          >
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full h-14 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-base font-semibold shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-0 mt-4"
            >
              {isLoading ? (
                <span className="flex items-center gap-2">
                  <Loader2 className="h-5 w-5 animate-spin" /> Processing
